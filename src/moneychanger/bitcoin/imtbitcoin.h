...
public:
    ...
    // returns new address
    virtual std::string GetNewAddress(const char* account = NULL) = 0;
    ...
    virtual std::string GetMultiSigAddress(int minSignatures, const std::list<std::string>& publicKeys, bool addToWallet = false, const char* account = NULL) = 0;

    // Creates a multi-sig address from public keys
    // returns an object containing info required to withdraw from that address
    virtual BtcMultiSigAddressRef GetMultiSigAddressInfo(int minSignatures, const std::list<std::string>& publicKeys, bool addToWallet = true, const char* account = NULL) = 0;
